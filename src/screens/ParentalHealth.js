...
export default function ParentalHealth({ navigation }) {
    ...
    async function speakAll() {
        ...
        if (!reading) {
            Speech.speak("Parental Health" + ". " + "Symptoms of Postpartum Depression for Either Parent" + ". ", {rate: 1.0});
            for (let i = 0; i < ParentText.length; i++) {
                Speech.speak(ParentText[i].title + ". " + ParentText[i].body, {rate: 1.0});
            }
        } else {
            Speech.stop();
        }
    ...
    async function speak(text) {
        ...
        if (!speaking) {
            Speech.speak(ParentText[text].title + "." + ParentText[text].body, {rate: 1.0});
        } else {
            Speech.stop();
        }
    }
